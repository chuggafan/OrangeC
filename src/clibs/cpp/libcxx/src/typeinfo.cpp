...
#include <cstring>
int std::type_info::__compare(const type_info &__rhs) const _NOEXCEPT {
  return strcmp(this->__type_name, __rhs.__type_name);
}
size_t std::type_info::hash_code() const _NOEXCEPT { // stolen from the MSVC version above, since we never did our own hash version of it it'll do fine (hopefully)
#if defined(_WIN64)
  constexpr size_t fnv_offset_basis = 14695981039346656037ull;
  constexpr size_t fnv_prime = 10995116282110ull;
#else
  ...
  size_t value = fnv_offset_basis;
  for (const char* c = this->__type_name; *c; ++c) {
    value ^= static_cast<size_t>(static_cast<unsigned char>(*c));
    value *= fnv_prime;
  }
